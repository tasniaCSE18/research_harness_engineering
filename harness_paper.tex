...
Structured outputs constrain a model to emit JSON conforming to a provided JSON Schema, which is ideal for data extraction and classification \cite{b4}. Schema guidance gives two distinct benefits: it guarantees parseable output and it focuses the model's attention on exactly the fields the application needs. In our design the schema is part of the harness---it is an optimizable artifact rather than a fixed input.
...
LLM self-refinement---asking a model to critique and regenerate its own output---has produced mixed results; without an external verifier, iterative self-correction can plateau or regress \cite{b3}. Our design therefore restricts correction to \emph{targeted re-reading of low-confidence fields}, which is a narrower and more auditable operation than full regeneration.
...
With a single structured call, the harness already reaches 83.3\% field-level exact match and 98.3\% character-level F1 (Table~\ref{tab:results}, Fig.~\ref{fig:metrics}). The remaining errors are near-misses---case differences such as \texttt{No.} vs.\ \texttt{NO.} or date-separator differences---rather than hallucinated content. This confirms that schema-guided structured output is a reliable foundation and that the interesting research questions live in confidence estimation and correction rather than raw decoding.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.2in]{fig/fig_metric_comparison.png}}
    \caption{Field-level exact match, character-level field F1, and document-level full match for the three harness variants on the pilot subset. All three metrics are identical across variants, indicating that added mechanisms did not yet move accuracy at this sample size.}
    \label{fig:metrics}
\end{figure}
...
On the pilot subset, the correction call re-affirmed the majority-vote values and did not change accuracy (Variant C equals Variant B in Table~\ref{tab:results}). This is consistent with the literature on self-refinement, which finds that correction without an external verifier often plateaus \cite{b3}. The value of the mechanism in our design is not that it always improves accuracy, but that it is \emph{targeted} (only low-confidence fields), \emph{bounded} (at most one extra call per flagged field), and \emph{auditable} (the harness records which fields were flagged and corrected). Whether targeted correction helps on a larger, harder set is an open question we plan to answer at full scale.

\subsection{Cost and Quota-Awareness}
Variant C costs at most \(N + 1\) calls per document (\(N\) samples plus up to one correction call) compared to one call for the baseline. Fig.~\ref{fig:cost} plots this trade-off directly: Variant B pays a \(3\times\) call budget over Variant A for the same accuracy on the pilot subset, and Variant C's targeted correction adds only a small additional cost (0.3 calls/doc, since just one of three documents had a flagged field) rather than a full extra pass. On the free tier this translates to a practical ceiling of roughly 30--50 documents per day for our design. This budget constraint is itself research-relevant: it motivates a budget-aware harness that spends extra calls only when confidence is low and the document is hard.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.0in]{fig/fig_cost_accuracy.png}}
    \caption{Model calls per document versus field-level exact match for each harness variant. Variant C's correction step adds a small, targeted cost over Variant B rather than a full additional pass.}
    \label{fig:cost}
\end{figure}
