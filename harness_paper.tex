...
\begin{abstract}
The reliability of vision-language model (VLM) systems for document intelligence depends not only on model weights but also on the \emph{harness} surrounding them: the code and configuration that decide what context the model sees, how its output is constrained, and when its answer is trusted. This study introduces a schema-guided, confidence-aware, and self-corrective harness for key-value extraction from real scanned documents. The harness fixes a frozen VLM and composes three reliability mechanisms: (1) schema-guided structured extraction that constrains the model output to a typed JSON Schema; (2) self-consistency confidence estimation that samples the model multiple times and measures per-field agreement; and (3) targeted self-correction that re-reads only low-confidence fields instead of blindly regenerating a full document. We evaluate the harness on the SROIE scanned-receipt benchmark and compare it against a single-call schema-only baseline. On a small three-document pilot---explicitly a directional smoke test of the pipeline rather than a statistically powered result---the harness reaches a field-level exact-match accuracy of 83.3\% and a character-level field F1 of 98.3\%, while the confidence signal correctly flags genuinely hard documents at the cost of a bounded number of additional model calls. A calibration analysis (expected calibration error of 0.28) reveals that raw agreement-based confidence is conservative but miscalibrated, motivating a calibration layer as a core research component. We cross-check these pilot magnitudes against independently published results on SROIE and closely related VLM document-extraction and self-consistency-calibration tasks and find them consistent in order of magnitude, giving external grounding beyond our own small run. The framework highlights how treating prompt, schema, sampling, and correction policy as a single optimizable harness can make document intelligence more reliable and auditable, and it exposes budget-aware constraints that matter for reproducible research on commodity API tiers.
\end{abstract}
...
\begin{itemize}
    ...
    \item We evaluate on real scanned receipts from the SROIE benchmark and report field-level exact match, character-level F1, document-level full match, and expected calibration error, showing that confidence signals flag genuinely hard documents.
    \item We show that raw agreement-based confidence is miscalibrated and motivate a calibration layer as a core component of trustworthy document intelligence.
\end{itemize}
...
Experiments use the Gemini 2.5 Flash model on the free API tier. Self-consistency samples use \(N=3\) with temperature \(0.5\); the baseline uses temperature \(0\); the correction threshold is \(\tau = 0.5\). The free tier imposes a strict daily quota on image-generation calls, so the experimental harness is cache-first: every successful response is cached, and quota-exhaustion (HTTP 429) is retried with backoff. This makes runs resumable and reproducible without re-spending calls. Table~\ref{tab:results} reports results on a pilot subset of \emph{three} receipts (12 field predictions per variant); the identical pipeline and code are already validated end-to-end on a 24-document sample manifest and scale directly to the full 126-document SROIE evaluation split once quota allows.

We are explicit about what this pilot can and cannot support. With \(n=3\) documents, a single field-level flip changes FieldExact by 8.3 percentage points, so the point estimates in Table~\ref{tab:results} should be read as a directional smoke test of the pipeline's correctness and behavior---not as a statistically powered comparison between variants. We therefore do not claim that Variant C is equivalent to Variant B in the general case, only that it produced identical output on this specific 3-document sample. To give the pilot external grounding rather than relying on the codebase's own small run in isolation, Section~\ref{sec:litcompare} compares our pilot numbers against independently published results on the same and closely related tasks.

\begin{table}[htb]
\centering
\caption{Pilot results on three SROIE receipts: field exact match, character-level field F1, document-level full match, calls per document, and expected calibration error (ECE). Variant A has no confidence estimate, so ECE is undefined.}
\label{tab:results}
\begin{tabular}{lcccc}
\hline
\textbf{Variant} & \textbf{FieldExact} & \textbf{FieldF1} & \textbf{DocFull} & \textbf{ECE}\\
\hline
A (schema-only) & 0.833 & 0.983 & 0.667 & ---\\
B (confidence) & 0.833 & 0.983 & 0.667 & 0.277\\
C (self-correct) & 0.833 & 0.983 & 0.667 & 0.277\\
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:litcompare} places our pilot numbers alongside independently published results on SROIE and closely related VLM document-extraction and calibration tasks. These are results reported by other authors on their own (typically much larger) evaluation sets, included here as external context for whether our pilot's magnitudes are plausible---not as a head-to-head benchmark run under identical conditions.

\begin{table}[htb]
\centering
\caption{Our pilot numbers alongside independently published results on SROIE / VLM document extraction and self-consistency calibration. External results are as reported by their respective papers, not reproduced by us.}
...
\hline
\textbf{This work (pilot, \(n{=}3\))} & SROIE T3 & F1 = 98.3\%\\
Variant A/B/C, zero fine-tuning & & Exact = 83.3\%\\
Variant B/C (self-consistency) & & ECE = 0.277\\
\hline
\end{tabular}
\end{table}

Fig.~\ref{fig:benchmark} shows this comparison visually, with our pilot bars drawn hatched to keep them visually distinct from the larger-scale published results.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.3in]{fig/fig_benchmark_comparison.png}}
    \caption{Published SROIE / receipt-extraction results (LayoutLM \cite{b8}, LayoutLMv2 \cite{b9}, a prompted Gemini VLM study \cite{b10}) alongside our pilot's field F1 and field-exact match. Hatched bars mark our own 3-document pilot; solid bars are other authors' own, larger-scale evaluations.}
    \label{fig:benchmark}
\end{figure}

Two observations follow. First, our character-level field F1 (98.3\%) and field-exact accuracy (83.3\%) sit in the same neighborhood as the fine-tuned layout-model literature (95--98\% F1) and the closest zero-fine-tuning VLM analogue (87.46\% accuracy) \cite{b8,b9,b10}, despite using no task-specific training data and only three documents---evidence that the pipeline is functioning correctly and that its accuracy is of a realistic order of magnitude, rather than an artifact of an unrepresentative 3-document sample. Second, our ECE of 0.277 falls at the upper (less well-calibrated) end of the 0.075--0.35 range reported for self-consistency confidence elsewhere in the LLM calibration literature \cite{b11}, as Fig.~\ref{fig:ece_lit} shows directly; this is consistent with structured multi-field extraction being a harder calibration target than the single-answer math/reasoning tasks most calibration studies evaluate, and it reinforces our conclusion that a calibration layer is necessary before this confidence signal can be reported as a probability. Confirming both observations at a statistically meaningful sample size is the immediate next step (Section~\ref{sec:conclusion}).

\begin{figure}[htb]
\centerline{\includegraphics[width=3.3in]{fig/fig_ece_literature_context.png}}
    \caption{Our pilot's ECE (0.277) against published self-consistency-confidence ECE values and their single-sample p(True) baselines \cite{b11}. Our value is higher than the self-consistency range reported for math reasoning, motivating a task-specific calibration layer.}
    \label{fig:ece_lit}
\end{figure}

\subsection{Schema Guidance Is a Strong Baseline}
With a single structured call, the harness already reaches 83.3\% field-level exact match and 98.3\% character-level F1 on the pilot subset (Table~\ref{tab:results}). The remaining errors are near-misses---case differences such as \texttt{No.} vs.\ \texttt{NO.} or date-separator differences---rather than hallucinated content. This confirms that schema-guided structured output is a reliable foundation and that the interesting research questions live in confidence estimation and correction rather than raw decoding.

\subsection{Confidence Flags Genuinely Hard Documents}
The one document where all four fields fell below the confidence threshold (agreement \(0.33\) across three samples) was exactly the document whose single-call output contained formatting drift. In contrast, the two documents with agreement \(0.67\) were extracted correctly. This is a directional result (sample size three) that the confidence signal separates hard documents from easy ones, which is precisely the property a ``route to human review'' policy needs. Fig.~\ref{fig:err} breaks down the remaining Variant C errors by field: only \texttt{date} and \texttt{address} contribute mistakes, while \texttt{company} and \texttt{total} are extracted perfectly on the pilot subset.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.0in]{fig/fig_error_by_field.png}}
    \caption{Per-field error counts for Variant C on the pilot subset. Errors concentrate on \texttt{date} (format drift) and \texttt{address} (case drift), not on \texttt{company} or \texttt{total}.}
    \label{fig:err}
\end{figure}

\subsection{Confidence Is Miscalibrated but Conservative}
The expected calibration error of 0.277 shows that agreement-based confidence is not a calibrated probability: fields with confidence near 0.67 are in fact correct roughly 83\% of the time in our pilot (Fig.~\ref{fig:calib}). Directionally the signal is \emph{conservative}---it understates reliability---which is safe for high-consequence routing, but it must be calibrated before it can be reported as a probability. This motivates a calibration layer (e.g., temperature scaling or binning on a small development set) as a core component of the harness.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.0in]{fig/fig_calibration.png}}
    \caption{Reliability diagram for Variant C: mean predicted confidence (self-consistency agreement) versus empirical accuracy, binned into deciles. Marker size is proportional to the number of field predictions in the bin. Points below the diagonal indicate conservative (under-confident) calibration.}
    \label{fig:calib}
\end{figure}

\subsection{Targeted Self-Correction Is a Bounded, Auditable Operation}
On the pilot subset, the correction call re-affirmed the majority-vote values and did not change accuracy (Variant C equals Variant B in Table~\ref{tab:results}). Notably, this is a much more benign outcome than the regressions Huang et al.\ \cite{b3} report for \emph{intrinsic}, unguided self-correction (drops of up to 34 percentage points, e.g., 75.8\%$\to$41.8\% on CommonSenseQA); our correction call is not blind---it is triggered only for fields the self-consistency signal already flagged, and it re-affirmed rather than corrupted those fields. Fig.~\ref{fig:selfcorrect} places our result relative to both failure modes reported in the literature. This is consistent with the interpretation, supported by \cite{b3}'s oracle-guided results, that correction is safe and potentially beneficial when guided by an external signal, and harmful mainly when the model corrects itself with no signal at all. The value of the mechanism in our design is not that it always improves accuracy, but that it is \emph{targeted} (only low-confidence fields), \emph{bounded} (at most one extra call per flagged field), and \emph{auditable} (the harness records which fields were flagged and corrected). Whether targeted correction measurably improves accuracy---as opposed to merely avoiding the regressions of blind correction---on a larger, harder set is an open question we plan to answer at full scale.

\begin{figure}[htb]
\centerline{\includegraphics[width=3.3in]{fig/fig_selfcorrection_literature.png}}
    \caption{Accuracy change from a correction call: intrinsic/unguided self-correction regresses accuracy by up to 34 points, oracle-guided correction improves it by 8--14 points \cite{b3}, and our confidence-gated targeted correction (n=3) is flat---avoiding the unguided failure mode without yet demonstrating the oracle-guided gain.}
    \label{fig:selfcorrect}
\end{figure}

\subsection{Cost and Quota-Awareness}
Variant C costs at most \(N + 1\) calls per document (\(N\) samples plus up to one correction call) compared to one call for the baseline. On the pilot subset, Variant B pays a \(3\times\) call budget over Variant A for the same accuracy, and Variant C's targeted correction adds only a small additional cost (0.3 calls/doc, since just one of three documents had a flagged field) rather than a full extra pass. On the free tier this translates to a practical ceiling of roughly 30--50 documents per day for our design. This budget constraint is itself research-relevant: it motivates a budget-aware harness that spends extra calls only when confidence is low and the document is hard.

\section{Conclusion and Future Works}
\label{sec:conclusion}

This research successfully demonstrates a harness-engineering approach to reliable document intelligence. By fixing the VLM and varying only the surrounding harness---schema, sampling policy, confidence estimator, and correction rule---we isolate and measure the contribution of each reliability mechanism on real scanned receipts. Schema-guided structured extraction provides a strong baseline (83.3\% field exact match, 98.3\% character-level F1 on the pilot subset), self-consistency confidence correctly flags genuinely hard documents, and a calibration analysis quantifies the gap between agreement-based confidence and true accuracy.
...
\begin{itemize}
    \item Scale the evaluation to the full SROIE split and additional document types (forms and invoices) once API quota allows, and report confidence intervals.
    \item Compare self-consistency confidence against verbalized confidence and an external verifier model, and add a calibration layer on a held-out development set.
    \item Add a date/amount normalizer so that semantically correct near-miss extractions are not penalized by exact-match metrics.
